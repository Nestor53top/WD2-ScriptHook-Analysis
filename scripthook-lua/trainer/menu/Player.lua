...
-- Player Model Changer - by Kamzik123 --
local function ModelSelectMenu(menu, name, text)
	local menu = UI.SimpleMenu()
	menu:SetTitle("Model Selection")
	menu:AddSearch("Search", "Start typing to filter")
	
	for k,v in pairsByKeys(GraphicKits[name]) do
		menu:AddButton(k,v, function()
			ScriptHook.SetEntityGraphicKitModel(GetLocalPlayerEntityId(), v)
		end)
	end
...
local function PartsMenu(menu, name, text)
	local menu = UI.SimpleMenu()
	menu:SetTitle("Graphic Kits")
	
	for k,v in pairsByKeys(GraphicKits) do
		menu:AddButton(k, "Graphic Kits " .. k, ModelSelectMenu)
	end
...
local function PlayerMenu()
	...
	menu:AddButton("Change Model", PartsMenu)
